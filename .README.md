...
- `js/core.js` — **núcleo lógico compartido** (fechas, tier/destacados, canales,
  ilustraciones). Lo usa la vista de TV; la web puede migrar a él más adelante.
- `tv.html` + `css/tv.css` + `js/tv.js` — **vista de TV (10-foot UI)**: navegación con
  mando (flechas + OK), foco muy visible, texto grande. Reutiliza `css/tokens.css`, los
  assets y los datos. Los favoritos se comparten con la web (mismo `localStorage`).
  Accesible en `/tv.html`.
- `favicon.ico` + `assets/icons/` — logo y favicon (SVG + PNG 16/32/48/180/192/512).
